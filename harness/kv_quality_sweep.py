...
KNOWN IMPRECISION: filler is built once per ctx, sized against the probe with
the largest artifact+question token count, then trimmed shorter per-probe via
left_truncate_tokens(). Trimming only ever shortens; it cannot extend a filler
that is already shorter than a given probe needs. Since only one probe (the
one that set the sizing target) has an artifact+question pairing at the
maximum, every other probe ends up filled to slightly less than 90% -- by
observation, up to ~150 tokens short (~0.5% of ctx=32768, ~2% of ctx=8192).
The actual token count achieved is always recorded per-row in
n_prompt_tokens_actual, so this is visible and auditable, not silently wrong,
but it means "90% fill" is a target, not an exact figure, for most rows.
"""
