...
| `results/t2s_m3_power_coupling_20260925T075348Z.jsonl` (+ manifest, `_sysman.csv`, `_wincounters.jsonl`, `_spin*_hog_*.{json,txt}`, `.DONE`) | evo-t2s (Intel Core Ultra X7 358H, Arc B390, Vulkan b10970, unified memory) | **NO** | **M3: CPU spin slows the iGPU through a shared package power limit; P-core-heavy prediction rejected** | ctx 8192, 3 measured calls per condition after one warm-up; none, spinE (0xFFF0), spin16, spinP (0x000F), none_end; hog affinity read-back equals the requested mask on every worker; no invalid rows. Package power at about 45 W in every spin condition, iGPU at its power-limited 1650 MHz on 100% of samples under spinE and spin16 (TTFT 1.42x and 1.41x) but 32% under spinP (1.04x). Scripts `harness/t2s_m3_power_coupling.py`, `harness/level_zero_sysman.py`, `harness/spin_hog_affinity.py`, `harness/server_guard.py`, `harness/run_provenance.py`, committed in 144f792 and verified on the machine against their git blobs (manifest `script_provenance`, head 2ce23fd); analysis `analysis/t2s_m3_analysis.py` (757a2c2). A smoke run of one spin16 condition preceded it and is not committed (scratch only). Sysman struct layouts and throttle-bit meanings are from `zes_api.h` as recalled and unverified on the machine; see limits in `docs/FINDINGS.md` (M3 section). |

| `results/blade_c1_spill_sweep_20260925T053651Z.jsonl` (+ manifest, `_smi.csv`, `_dmon.txt`, `_winctr.csv`, `.DONE`, `results/blade_c1_server_logs/`) | blade_rtx4070 (Razer Blade 14, RTX 4070 Laptop, 8188 MiB VRAM, CUDA b10970, driver 610.88) | **NO** | **C1: VRAM spill quantified, VALID** | ctx 32768 to 47104 step 2048, 1 warm-up plus 5 measured calls per ctx, grid order shuffled (seed 20260925), 4 anchors with drift at most 0.5%, no failed or invalid call, no failure through ctx 47104. Onset between ctx 36864 (0.98x) and 38912 (3.71x); decode slowdown = 0.08 + 74.6 x excess spilled fraction (R2 0.998). Table, fits and limits in `docs/FINDINGS.md` (C1 section). Script `harness/blade_spill_sweep.py` and `harness/blade_telemetry.py` at commit 1c17f5d (the manifest does not record the SHA); analysis `analysis/blade_c1_spill_analysis.py` (bf32465). Stale-server audit: 6 request markers in each of the 11 server logs. Ran before the standing guard existed (own start-time listener check only). |

| `results/blade_c2_spill_correctness_20260925T110739Z.jsonl` (+ manifest, telemetry, `.DONE`, `results/blade_c2_server_logs/`) | blade_rtx4070 (Razer Blade 14, RTX 4070 Laptop, 8188 MiB VRAM, CUDA b10970, driver 610.88) | **NO** | **C2: correctness under spill, VALID** | art_01 to art_05 at ctx 36864 (no spill) and ctx 47104 (14.2% spilled), 10 of 10 correct, identical outputs across the two contexts. Script `harness/blade_spill_sweep.py` (last change c6ab4a9) with `harness/blade_telemetry.py`, `harness/server_guard.py`, `harness/run_provenance.py`; the manifest records git head 9bfd881 and `committed: true`. Stale-server guard active, 5 request markers in each server log. See `docs/FINDINGS.md` (C2 section). |

| **Correction to the `ramlock_evo-t2s_20260925T010739Z.jsonl` entry above (2026-09-25)** | | | | The balloon's low-available safety valve fired at 215.3 s in level D3_S6 (row `safety_valve_low_available` in its balloon CSV), releasing the lock as the server finished loading, and the D3_S4 server working set (constant 9,011 MB after load) matches an unconstrained server, so the post-load results at S=6 and S=4 are INVALID as constrained measurements. The balloon logs also stop after about 100 s in every level (stdout pipe never read); the D2 gate therefore verified only the first 105 s. Levels S=12 to S=7 remain valid (working-set evidence of a held lock). Details in `docs/FINDINGS.md` (Phase D correction). |
