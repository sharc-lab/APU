...
| `results/kv_provisioning_20260924T003514Z_ctx32768_f16.jsonl` | evo-t2s (Intel Arrow Lake, Vulkan b10970-bfdc32183, unified LPDDR5X) | **NO** | **Provisioning comparison — f16@32768 leg (fast baseline)** | First of 2 legs comparing q4_0@ctx=131072 vs f16@ctx=32768 (same ~7.5–8.6 GiB resident memory budget: KV + 2991 MiB model weight). START position only, 4 probes (art_01, art_05, art_06, art_09), 1 rep each. 4/4 score 1.0. The q4_0@131072 leg is running on evo-t2s at time of writing (~5.3h estimated) and will be committed separately on completion. Script: `harness/kv_provisioning_sweep.py`. Same deviations as the quality sweep above apply (`--reasoning-format deepseek --reasoning-budget 0`; sys_free memory-delta precision check likely subject to the same negative-delta risk between the two legs, since `kill_server()` is unchanged from the quality sweep — to be confirmed once the second leg's delta is inspected). |
| `results/kv_neardup_*.jsonl` | evo-t2s | — | **Near-duplicate discrimination under KV quantization — DESIGNED, NOT RUN** | See `docs/FINDINGS.md` and `harness/kv_neardup_sweep.py`. Queued to run immediately after the provisioning comparison above completes. |
